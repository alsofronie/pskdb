...
function Transaction(pdsHandler) {
    this.add = function (swarm) {
        const meta = swarm.getMeta();
        if (swarm.alias) {

            pdsHandler.writeKey(meta.swarmTypeName + '/aliases/' + swarm.alias, meta.swarmId);
        }

        const serializedSwarm = beesHealer.asJSON(swarm, null, null);

        pdsHandler.writeKey(meta.swarmTypeName + '/' + meta.swarmId, J(serializedSwarm));
    };
